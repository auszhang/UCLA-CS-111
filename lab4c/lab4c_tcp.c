...
void fatal_usage() {
    fprintf(stderr, "Usage: lab4c_tcp portnum --id=number --host=name/address [--period=value] [--scale=[F,C]]] [--log=filename]\n");
    exit(EXIT_ARGS);
}
...
void write_server(char *message) {
    ssize_t ret = write(socketfd, message, strlen(message));
    if (ret < 0)
...
int main(int argc, char **argv) {
    ...
    while(1) {
        ...
        if (pollin >= 1 && generated_first_report) {
            ssize_t readin = read(socketfd, cmdbuf, sizeof(cmdbuf));
            if (readin > 0) {
                char *s = cmdbuf;
                // parse the command
